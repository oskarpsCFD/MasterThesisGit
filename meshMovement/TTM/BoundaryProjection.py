...
def build_polyline_tree(curve_xy, k=8):
    """
    Pre-compute everything needed for fast repeated projection onto a polyline.

    Call this once before the Winslow iteration loop and pass the returned
    dict to adjustBoundary / project_points_to_polyline via `tree_data`.
...
# ---------------------------------------------------------------------------
# Vectorised projection — no Python loop over query points
# ---------------------------------------------------------------------------

def project_points_to_polyline(P, curve_xy, k=8, tree_data=None):
    """
    ...
    tree_data  : dict from build_polyline_tree(), or None to build on the fly
    ...
    if tree_data is None:
        tree_data = build_polyline_tree(curve_xy, k=k)
...
def generateBoundaryPhiPsi(w1, w2, dxi, deta):
    # Use the same formula as the interior phi/psi in ThomasMiddlecoff.py:
    #   phi = (w[i+1] - w[i-1]) / (2 * w[i])
    # The interior formula does NOT divide by dxi.  Dividing by dxi here would
    # amplify the result by ~1/dxi (≈150×) relative to the interior values,
    # making the redistribution ODE far more aggressive than the Winslow solver
    # expects and causing extreme boundary clustering.
    phiBot = (w1[0, 2:] - w1[0, :-2]) / 2.0 / w1[0, 1:-1]
    phiTop = (w1[-1, 2:] - w1[-1, :-2]) / 2.0 / w1[-1, 1:-1]

    phiLeft  = (w2[2:, 0] - w2[:-2, 0]) / 2.0 / w2[1:-1, 0]
    phiRigth = (w2[2:, -1] - w2[:-2, -1]) / 2.0 / w2[1:-1, -1]

    return phiBot, phiTop, phiLeft, phiRigth
