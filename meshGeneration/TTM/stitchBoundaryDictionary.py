...
def confirmStitchRegion(X, Y):
    ...
    for index, boundary in enumerate(boundaries):
        ...
        if np.allclose(boundaryLine[0], boundaryLine[-1]):
            string += f"{boundary} is stitched\n"
            stitchArray[index] = True
        else:
            string += f"{boundary} is not stitched\n"
    return stitchArray, string


def locateStitchedIndices(P: NDArray):
    stitchedMap = []
    for index in range(1, P.shape[0] // 2 + 1):  # start at 1, skip trivial corner pair
        if np.allclose(P[index], P[-index-1], rtol=1e-8):
...
def createStitchBoundaryDict(X, Y):
    ...
    stitchMapDictionary = {}
    stitchArray, string = confirmStitchRegion(X, Y)
    ...
    return stitchMapDictionary, string