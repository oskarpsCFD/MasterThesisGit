...
def writeFoamHexMesh(vertices, hexes, filename="foamMesh.txt"):
    with open(filename, "w") as f:
        ...
        f.write("(\n")
        for h in hexes:
            indices = " ".join(str(int(i)) for i in h)
            f.write(f"  ({indices})\n")
        f.write(")\n")
    print(f"✅ Mesh successfully written to '{filename}'")

def _foamHeader(cls: str, location: str, obj: str) -> str:
    return f"""/*--------------------------------*- C++ -*----------------------------------*\
| =========                 |                                                 |
...
def writePointField(points, file_path, location="constant/structuredMesh", object_name=None):
    """
    ...
    with open(file_path, "w") as f:
        f.write(_foamHeader("vectorField", location, object_name))
        f.write(f"{len(pts)}\n(\n")
        for p in pts:
...
def writeLabelList(labels, file_path, location="constant/structuredMesh", object_name=None):
    """
    ...
    with open(file_path, "w") as f:
        f.write(_foamHeader("labelList", location, object_name))
        f.write(f"{len(arr)}\n(\n")
        for v in arr:
            f.write(f"{int(v)}\n")
        f.write(")\n")
...
def makeLogicalIJ(nx, ny, ordering="rowMajor"):
    """
...
def writeStructuredMeshFiles(
    ...
    logical_i, logical_j = makeLogicalIJ(nx, ny, ordering=ordering)
    ...
    writePointField(original_points, folder / "originalPoints")
    writeLabelList(original_to_foam_point, folder / "originalToFoamPoint")
    writeLabelList(logical_i, folder / "logicalI")
    writeLabelList(logical_j, folder / "logicalJ")
