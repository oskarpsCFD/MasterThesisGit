...
def clusterTowardStart(s: NDArray, beta: float = 3.0) -> NDArray:
    """One-sided tanh stretching toward s=0. Larger beta = tighter clustering."""
    s = f(s)
    return np.clip(1.0 - np.tanh(beta * (1.0 - s)) / np.tanh(beta), 0.0, 1.0)


def clusterTowardEnd(s: NDArray, beta: float = 3.0) -> NDArray:
    """One-sided tanh stretching toward s=1. Larger beta = tighter clustering."""
    s = f(s)
    return np.clip(np.tanh(beta * s) / np.tanh(beta), 0.0, 1.0)
